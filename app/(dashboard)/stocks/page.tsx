...
function StockLogo({ domain, logo_url, name, size = 44 }: { domain: string; logo_url: string; name: string; size?: number }) {
  ...
  return (
    <div
      ...
      className="flex items-center justify-center bg-[#f0f0ec] dark:bg-[#1e3827]"
    >
...
function StockCardSkeleton() {
  return (
    <div className="bg-white dark:bg-[#132b1a] border border-[#e5e5e5] dark:border-[#1e3827] rounded-xl p-5 animate-pulse">
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-xl bg-[#f0f0ea] dark:bg-[#1e3827]" />
          <div className="space-y-2">
            <div className="h-3.5 w-32 rounded bg-[#f0f0ea] dark:bg-[#1e3827]" />
            <div className="h-3 w-20 rounded bg-[#f0f0ea] dark:bg-[#1e3827]" />
          </div>
        </div>
        <div className="w-20 h-8 rounded bg-[#f0f0ea] dark:bg-[#1e3827]" />
      </div>
      <div className="flex items-end justify-between">
        <div className="space-y-2">
          <div className="h-6 w-24 rounded bg-[#f0f0ea] dark:bg-[#1e3827]" />
          <div className="h-3 w-16 rounded bg-[#f0f0ea] dark:bg-[#1e3827]" />
        </div>
        <div className="space-y-1 text-right">
          <div className="h-2.5 w-8 rounded bg-[#f0f0ea] dark:bg-[#1e3827]" />
          <div className="h-3 w-12 rounded bg-[#f0f0ea] dark:bg-[#1e3827]" />
        </div>
      </div>
    </div>
  );
    ...
    <div className="flex items-center gap-0 overflow-x-auto">
      {[1, 2, 3, 4].map((i) => (
        <div key={i} className={`py-3 pr-6 ${i > 1 ? "pl-6" : ""} shrink-0 animate-pulse`}>
          <div className="h-2.5 w-16 rounded bg-[#e0e0d8] dark:bg-[#1e3827] mb-2" />
          <div className="h-4 w-24 rounded bg-[#e0e0d8] dark:bg-[#1e3827]" />
        </div>
      ))}
    </div>
  );
      ...
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-[13px] font-bold text-[#001011] dark:text-[#f0f0f0] leading-tight truncate group-hover:text-[#0c5c45] dark:group-hover:text-[#34d399] transition-colors">
              {stock.name}
            </p>
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold tracking-wide bg-[#eaf5f0] dark:bg-[#1e3827] text-[#0c5c45] dark:text-[#34d399]">
                {stock.symbol}
              </span>
              {sectorMeta && (
                <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium">
                  <span className="dark:hidden" style={{ color: sectorMeta.text, backgroundColor: sectorMeta.bg, padding: "2px 6px", borderRadius: 4 }}>{stock.sector}</span>
                  <span className="hidden dark:inline" style={{ color: sectorMeta.darkText, backgroundColor: sectorMeta.darkBg, padding: "2px 6px", borderRadius: 4 }}>{stock.sector}</span>
                </span>
              )}
            </div>
          </div>
      ...
      >
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-bold bg-[#eaf5f0] dark:bg-[#1e3827] text-[#0c5c45] dark:text-[#34d399]">
                  {stock.symbol}
                </span>
              </div>
        ...
        <div className="px-6 pt-5 pb-2">
          ...
          <div className="flex items-center justify-between mt-2 px-1">
            <span className="text-[10px] text-[#aaaaaa] dark:text-[#4a6655]">20 days ago</span>
        ...
        <div className="px-6 py-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {metrics.map(({ label, value }) => (
              <div key={label} className="bg-[#f8f8f5] dark:bg-[#132b1a] border border-[#eeeeea] dark:border-[#1e3827] rounded-lg px-3 py-2.5">
          ...
          <div className="flex items-center gap-2 mb-4 flex-wrap">
            {sectorMeta && (
              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-semibold">
                <span className="dark:hidden" style={{ color: sectorMeta.text, backgroundColor: sectorMeta.bg, padding: "4px 10px", borderRadius: 999 }}>{stock.sector}</span>
                <span className="hidden dark:inline" style={{ color: sectorMeta.darkText, backgroundColor: sectorMeta.darkBg, padding: "4px 10px", borderRadius: 999 }}>{stock.sector}</span>
              </span>
            )}
            {stock.exchange && (
            ...
            <div className="flex items-center gap-0 overflow-x-auto scrollbar-hide">
              ...
              <div className="ml-auto pl-6 border-l border-[#d8d8d0] dark:border-[#1e3827] shrink-0 py-3">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] font-medium text-[#16a34a] dark:text-[#22c55e]">Live</span>
                </div>
      ...
      <div className="max-w-[1360px] mx-auto px-4 lg:px-6 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <div className="relative sm:w-[280px]">
            ...
            <input
              ...
              className="w-full h-10 pl-9 pr-4 border border-[#e5e5e5] dark:border-[#1e3827] bg-white dark:bg-[#0e1e14] text-[13px] text-[#001011] dark:text-[#f0f0f0] placeholder:text-[#aaaaaa] dark:placeholder:text-[#4a6655] rounded-lg outline-none focus:border-[#0c5c45] dark:focus:border-[#34d399] transition-colors"
            />
            {searchQuery && (
              <button
        ...
        <div className="flex items-center gap-2 flex-wrap mb-6">
          {SECTOR_FILTERS.map((sector) => (
            <button
              ...
              className={`h-8 px-4 rounded-full text-[12px] font-semibold transition-all duration-150 ${
                activeFilter === sector
                  ? "bg-[#0c5c45] text-white shadow-sm"
                  : "border border-[#e5e5e5] dark:border-[#1e3827] text-[#555555] dark:text-[#8fa896] bg-white dark:bg-transparent hover:border-[#0c5c45] dark:hover:border-[#34d399] hover:text-[#001011] dark:hover:text-[#34d399]"
              }`}
            >
              {sector}
            </button>
          ...
          {!isLoading && (
            <span className="ml-auto text-[12px] text-[#888888] dark:text-[#8fa896]">
              Showing <span className="font-bold text-[#001011] dark:text-[#f0f0f0]">{filtered.length}</span> of{" "}
              <span className="font-bold text-[#001011] dark:text-[#f0f0f0]">{stocks.length}</span> stocks
              {totalPages > 1 && <> &nbsp;·&nbsp; Page <span className="font-bold text-[#001011] dark:text-[#f0f0f0]">{currentPage}</span> of <span className="font-bold text-[#001011] dark:text-[#f0f0f0]">{totalPages}</span></>}
            </span>
          )}
        </div>
        ...
        {isLoading ? (
              ...
              <div className="flex items-center justify-center gap-2 mt-8">
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 h-9 rounded-full text-[13px] font-semibold border transition-all
                    border-[#e5e5e5] dark:border-[#1e3827] bg-white dark:bg-transparent
                    text-[#555555] dark:text-[#8fa896]
                    hover:border-[#0c5c45] dark:hover:border-[#34d399] hover:text-[#001011] dark:hover:text-[#34d399]
                    disabled:opacity-35 disabled:cursor-not-allowed disabled:hover:border-[#e5e5e5] disabled:hover:text-[#555555] dark:disabled:hover:border-[#1e3827] dark:disabled:hover:text-[#8fa896]"
                >
                ...
                <div className="flex items-center gap-1">
                  {Array.from({ length: totalPages }, (_, i) => i + 1)
                    ...
                    .map((p, i) =>
                      p === "…" ? (
                        <span key={`ellipsis-${i}`} className="w-9 h-9 flex items-center justify-center text-[13px] text-[#888888] dark:text-[#4a6655]">…</span>
                      ) : (
                        <button
                          key={p}
                          ...
                          className={`w-9 h-9 rounded-full text-[13px] font-semibold transition-all ${
                            currentPage === p
                              ? "bg-[#0c5c45] text-white shadow-sm"
                              : "border border-[#e5e5e5] dark:border-[#1e3827] bg-white dark:bg-transparent text-[#555555] dark:text-[#8fa896] hover:border-[#0c5c45] dark:hover:border-[#34d399] hover:text-[#001011] dark:hover:text-[#34d399]"
                          }`}
                        >
                          {p}
                        </button>
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-[15px] font-semibold text-[#001011] dark:text-[#f0f0f0] mb-1">No data yet</p>
            <p className="text-[13px] text-[#888888] dark:text-[#8fa896]">
              Run <code className="bg-[#f0f0ea] dark:bg-[#1e3827] px-1 rounded">fetch_stock_quotes</code> and{" "}
              <code className="bg-[#f0f0ea] dark:bg-[#1e3827] px-1 rounded">fetch_stock_data</code> to seed prices.
            </p>
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-[13px] text-[#888888] dark:text-[#8fa896]">Try adjusting your search or filter</p>
          </div>
        )}
      </div>
