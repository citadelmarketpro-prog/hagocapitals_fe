...
function Sk({ className }: { className?: string }) {
  return (
    <div className={`animate-pulse rounded bg-[#e0dfd4] dark:bg-[#1a2e1e] ${className ?? ""}`} />
  );
}
    ...
    <div className="min-h-screen dark:bg-[#0c1a10] flex flex-col">
      ...
      <main className="flex-1 px-4 py-4 lg:px-6 lg:py-5">
        <div className="max-w-[1360px] mx-auto flex flex-col gap-4">
          ...
          {/* ── Greeting ── */}
          <div>
            <h1 className="text-[20px] font-bold text-[#001011] dark:text-white leading-tight">
              {getGreeting()}, {firstName}
            </h1>
            <p className="text-[13px] text-[#888888] dark:text-[#5a7060] mt-0.5">
  ...
  return (
    <div className="fixed sm:absolute top-[56px] sm:top-full sm:mt-2 left-0 right-0 sm:left-auto sm:right-0 w-full sm:w-[340px] bg-white dark:bg-[#0e1e14] border-b sm:border border-[#e5e5e5] dark:border-[#1e3827] shadow-xl z-50 max-h-[calc(100vh-56px)] sm:max-h-[480px] overflow-y-auto">
      <div className="sticky top-0 flex items-center gap-2 px-4 py-3 bg-white dark:bg-[#0e1e14] border-b border-[#f0f0ec] dark:border-[#1a2e1e]">
        ...
        {unread > 0 && (
          <button onClick={markAllRead} className="ml-auto text-[12px] text-[#555555] dark:text-[#8fa896] hover:text-[#001011] dark:hover:text-white transition-colors whitespace-nowrap">
            Mark all as read
          </button>
        )}
        <button onClick={onClose} className="w-6 h-6 rounded-full flex items-center justify-center bg-[#f0f0ec] dark:bg-[#1a2a1e] text-[#888888] hover:text-[#001011] dark:hover:text-white transition-colors ml-auto shrink-0">
          <CloseIcon />
        </button>
      </div>
      {loading ? (
        <div className="px-4 py-8 text-center text-[13px] text-[#aaaaaa] dark:text-[#4a6655]">Loading…</div>
      ) : notifs.length === 0 ? (
        <div className="px-4 py-10 text-center text-[13px] text-[#aaaaaa] dark:text-[#4a6655]">No notifications yet</div>
      ) : notifs.map((n) => (
        <div key={n.id} onClick={() => { if (!n.is_read) markOneRead(n.id); }}
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[13px] font-semibold text-[#001011] dark:text-white leading-snug mb-0.5">{n.title}</p>
  ...
  return (
    <div className="fixed sm:absolute top-[56px] sm:top-full sm:mt-2 left-0 right-0 sm:left-auto sm:right-0 w-full sm:w-[290px] bg-white dark:bg-[#0e1e14] border-b sm:border border-[#e5e5e5] dark:border-[#1e3827] shadow-xl z-50">
      ...
      <div className="bg-[#f5f5ef] dark:bg-[#132b1a] border-b border-[#e5e5e5] dark:border-[#1e3827] px-4 py-3 flex items-center justify-between cursor-pointer hover:bg-[#eeeee9] dark:hover:bg-[#1a3020] transition-colors">
        <span className="text-[13px] font-medium text-[#001011] dark:text-white">About HagoCapitals</span>
        <ExternalIcon />
      </div>
        ...
        <div className="px-6 py-7">
          ...
          <div className="flex flex-col items-center mb-6">
            ...
            <div className="flex items-center gap-2">
              <button className="h-8 px-4 rounded-full border border-[#ef4444] text-[12px] font-medium text-[#ef4444] hover:bg-[#fff5f5] dark:hover:bg-[#2a1515] transition-colors">
                Remove
              </button>
              <button className="h-8 px-4 rounded-full border border-[#d4d4d4] dark:border-[#2a4a34] text-[12px] font-medium text-[#555555] dark:text-[#8fa896] hover:bg-[#f5f5f5] dark:hover:bg-[#1a2a1e] transition-colors">
                Change
              </button>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full h-10 px-3 border border-[#e5e5e5] dark:border-[#1e3827] bg-white dark:bg-[#0b1c11] text-[13px] text-[#001011] dark:text-white outline-none focus:border-[#0c5c45] dark:focus:border-[#34d399] transition-colors"
                />
              </div>
              <div>
                <label className="block text-[12px] font-medium text-[#333333] dark:text-[#8fa896] mb-1.5">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full h-10 px-3 pr-9 border border-[#e5e5e5] dark:border-[#1e3827] bg-white dark:bg-[#0b1c11] text-[13px] text-[#001011] dark:text-white outline-none focus:border-[#0c5c45] dark:focus:border-[#34d399] transition-colors"
                />
          ...
          {/* Submit */}
          <button className="w-full h-11 mt-6 rounded-full text-[13px] font-bold bg-[#0c5c45] text-white hover:opacity-90 transition-opacity">
            Update information
          </button>
        </div>
      </div>
...
function KycBadge({ status }: { status: string | null }) {
  ...
  if (status === "approved") {
    return (
      <span className="inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-[10px] font-semibold w-fit bg-[rgba(12,92,69,0.1)] dark:bg-[rgba(52,211,153,0.12)] border border-[rgba(12,92,69,0.2)] dark:border-[rgba(52,211,153,0.25)] text-[#0c5c45] dark:text-[#34d399]">
        ✓ Verified
      </span>
    );
  }
...
function PortfolioCard({ kycStatus }: { kycStatus: string | null }) {
  ...
  const todayColor    = todayUp ? "#22a34a" : "#dc2626";
      ...
      <div className="overflow-hidden bg-[#e5e3d5] dark:bg-[#0f1e14] border border-[#d0cec0] dark:border-[#1e3827]">
        ...
        <div className="px-5 pt-5 pb-5">
          ...
          <div className="flex items-center justify-between mb-5">
            ...
            {/* LIVE indicator */}
            <div className="flex items-center gap-1.5 rounded-full px-3 py-1.5 bg-[rgba(42,90,60,0.08)] dark:bg-[rgba(94,201,135,0.1)] border border-[rgba(42,90,60,0.15)] dark:border-[rgba(94,201,135,0.2)]">
              <span className="relative flex w-2 h-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75" />
                <span className="relative inline-flex w-2 h-2 rounded-full bg-emerald-500" />
              </span>
              <span className="text-[11px] font-bold tracking-wide text-[#2a5a3c] dark:text-[#5ec987]">LIVE</span>
            </div>
          </div>

          {/* ── Balance label ── */}
          <div className="text-[11px] font-semibold uppercase tracking-[0.7px] text-[#888888] dark:text-[#5a7060] mb-1.5">
            Total Balance
          </div>
            ...
            <div className="relative flex items-start gap-2">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-2 mb-4">
                  ...
                  <span className="text-[11px] font-semibold text-[#888888] dark:text-[#5a7060]">
...
{/* ── 4-button action grid ── */}
        <div className="grid grid-cols-4 gap-2 px-4 pb-5 pt-4 border-t border-[#d0cec0] dark:border-[#1e3827]">

          {/* Deposit — primary */}
          <button
            onClick={() => setModal("addFunds")}
            className="flex flex-col items-center gap-1.5 rounded-[14px] p-3 transition-all hover:-translate-y-0.5 active:scale-95 bg-[#2a5a3c] dark:bg-[#2a5a3c]"
          >
          ...
          <button
            ...
            className="flex flex-col items-center gap-1.5 rounded-[14px] p-3 transition-all hover:-translate-y-0.5 active:scale-95 bg-[#f0efe6] dark:bg-[#1a2a1e]"
          >
            <div className="w-7 h-7 rounded-[9px] flex items-center justify-center bg-[rgba(42,90,60,0.12)] dark:bg-[rgba(94,201,135,0.1)]">
              <svg viewBox="0 0 14 14" fill="none" stroke="#2a5a3c" strokeWidth="2" strokeLinecap="round" className="w-3.5 h-3.5 dark:stroke-[#5ec987]">
                <path d="M7 13V4M3 8l4-4 4 4M1 1h12" />
              </svg>
            </div>
            <span className="text-[11px] font-bold text-[#2a5a3c] dark:text-[#5ec987]">Withdraw</span>
          </button>
          ...
          >
            ...
            <span className="text-[11px] font-bold text-[#2a5a3c] dark:text-[#5ec987]">Transfer</span>
          </button>
          ...
          >
            ...
            <span className="text-[11px] font-bold text-[#2a5a3c] dark:text-[#5ec987]">History</span>
          </button>
...
function TransferModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-[#0e1e14] w-full max-w-[340px] rounded-2xl overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {success ? (
          <div className="px-6 py-7 flex flex-col items-center text-center">
            ...
            <h2 className="text-[16px] font-bold text-[#001011] dark:text-white mb-2">Transfer successful</h2>
            <p className="text-[12px] text-[#888888] dark:text-[#4a6655] mb-1">
              Balance: <span className="font-semibold text-[#001011] dark:text-white">${fmt(balance)}</span>
            </p>
            <p className="text-[12px] text-[#888888] dark:text-[#4a6655] mb-6">
              Profit: <span className="font-semibold text-[#001011] dark:text-white">${fmt(profit)}</span>
            </p>
            <button
              onClick={onClose}
              className="w-full h-10 rounded-full text-[13px] font-medium text-[#001011] dark:text-white border border-[#e5e5e5] dark:border-[#1e3827] bg-white dark:bg-transparent hover:bg-[#f8f8f8] dark:hover:bg-[#132b1a] transition-colors"
            >
              Done
            </button>
          </div>
          ...
          <>
            ...
            <div className="flex items-center justify-between px-6 pt-6 pb-4">
              <h2 className="text-[16px] font-bold text-[#001011] dark:text-white">Transfer funds</h2>
              <button
                onClick={onClose}
                className="w-7 h-7 rounded-full flex items-center justify-center bg-[#f0f0ec] dark:bg-[#1a2a1e] text-[#888888] hover:text-[#001011] dark:hover:text-white transition-colors"
              >
              ...
              {loading ? (
                ...
                <>
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between px-4 py-3 bg-[#f5f5ef] dark:bg-[#1a2a1e]">
                      <div>
                        <p className="text-[10px] font-semibold uppercase tracking-wider text-[#888888] dark:text-[#5a7060] mb-0.5">From</p>
                        <p className="text-[13px] font-bold text-[#001011] dark:text-white">{fromLabel}</p>
                        <p className="text-[11px] text-[#888888] dark:text-[#5a7060]">Available: ${fmt(fromValue)}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between px-4 py-3 bg-[#f5f5ef] dark:bg-[#1a2a1e]">
                      <div>
                        <p className="text-[10px] font-semibold uppercase tracking-wider text-[#888888] dark:text-[#5a7060] mb-0.5">To</p>
                        <p className="text-[13px] font-bold text-[#001011] dark:text-white">{toLabel}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-center gap-1 py-3">
                    <input
                      ...
                      className="text-[44px] font-bold text-[#aaaaaa] dark:text-[#4a6655] bg-transparent border-none outline-none text-center w-full placeholder:text-[#aaaaaa] dark:placeholder:text-[#4a6655]"
                    />
                    <button
                      onClick={() => setAmount(String(fromValue))}
                      className="text-[11px] font-semibold text-[#2a5a3c] dark:text-[#5ec987] hover:underline"
                    >
                      MAX
                    </button>
                  </div>
    ...
    <div className="bg-white dark:bg-[#0e1e14] border border-[#e5e5e5] dark:border-[#1e3827] overflow-hidden">
      ...
      >
        ...
        <svg
          ...
          className={`text-[#888888] dark:text-[#5a7060] transition-transform duration-200 ${open ? "rotate-180" : ""}`}
        >
      ...
      {open && (loading ? (
        <div className="px-5 pb-5 space-y-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="border border-[#f0f0ec] dark:border-[#1a2e1e] p-4 space-y-3">
        ...
        <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
          ...
          <a
            href="/traders"
            className="h-10 px-8 flex items-center justify-center text-[13px] font-bold text-white hover:opacity-90 transition-opacity"
            style={{ backgroundColor: "#0c5c45" }}
          >
            Explore Traders
          </a>
        </div>
  ...
  return (
    <div className="bg-white dark:bg-[#0e1e14] border border-[#e5e5e5] dark:border-[#1e3827] px-5 py-4">
      <button
        ...
        className="flex items-center justify-center w-full sm:w-fit gap-2.5 h-10 px-4 rounded-full text-[14px] font-semibold text-[#001011] dark:text-[#f0f0f0] bg-[#fff5f5] dark:bg-[#2a1515] border border-[#fca5a5] dark:border-[#7f1d1d] hover:opacity-90 transition-opacity cursor-pointer"
      >
          ...
          {bars.map((item) => (
            <div key={item.category} className="flex items-center gap-2">
              ...
              <div className="flex flex-col leading-none gap-0.5">
                <span className="text-[11px] text-[#555555] dark:text-[#8fa896]">
                  {item.label}
                </span>
...
function FollowingCard({ copying, loading }: { copying: CopyingTrader[]; loading: boolean }) {
  ...
  return (
    <div className="bg-white dark:bg-[#0e1e14] border border-[#e5e5e5] dark:border-[#1e3827]">
      ...
      >
        <h2 className="text-[16px] font-bold text-[#001011] dark:text-white">Following</h2>
        <svg
          width="16" height="16" viewBox="0 0 16 16" fill="none"
          stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"
          className={`text-[#888888] dark:text-[#5a7060] transition-transform duration-200 ${open ? "rotate-180" : ""}`}
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          className="w-full h-9 pl-8 pr-3 border border-[#e5e5e5] dark:border-[#1e3827] bg-[#fafaf8] dark:bg-[#0b1c11] text-[13px] text-[#001011] dark:text-white placeholder-[#aaaaaa] dark:placeholder-[#4a6655] outline-none focus:border-[#0c5c45] dark:focus:border-[#34d399] transition-colors"
        />
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-8 text-center">
          ...
          <a
            href="/traders"
            className="h-10 px-6 flex items-center justify-center text-[13px] font-bold text-white hover:opacity-90 transition-opacity"
            style={{ backgroundColor: "#0c5c45" }}
          >
            Explore Traders
          </a>
